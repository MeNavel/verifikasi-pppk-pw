...
new class extends Component {
    ...
    // Variabel untuk modal catatan manual
    public bool $modalTolak = false;
    public string $catatanTolak = '';

    public array $opdOptions = [
        ['id' => '08', 'name' => 'Badan Pendapatan Daerah'],
        ['id' => '04', 'name' => 'Satuan Polisi Pamong Praja'],
        ['id' => '12', 'name' => 'Dinas Kependudukan dan Pencatatan Sipil'],
        ['id' => '23', 'name' => 'Dinas Pendidikan'],
        ['id' => '68', 'name' => 'Dinas Kesehatan, Pengendalian Pendudukan dan Keluarga Berencana'],
        ['id' => '36', 'name' => 'Kecamatan Ambulu'],
        ['id' => '45', 'name' => 'Kecamatan Kaliwates'],
        ['id' => '60', 'name' => 'Kecamatan Sumberjambe'],
    ];
...
//        'berkas3'  => ['label' => 'Transkrip Nilai', 'ver' => 'ver3', 'cat' => 'cat3', 'opsi' => ['Dokumen tidak sesuai', 'Dokumen tidak terbaca']],
    ...
    {
        $targetNips = DB::table('target_nips_bagus')->pluck('nip')->toArray();
    ...
    // Fungsi jika menggunakan tombol tolak dari opsi (Berlaku untuk KK, SK, dsb)
    public function tolakDenganCatatan(int $opsiIndex): void
    {
        ...
        $this->prosesTolak($catatanText);
    }

    // Fungsi jika menggunakan modal catatan (Berlaku untuk SKP)
    public function simpanPenolakan(): void
    {
        if (empty(trim($this->catatanTolak))) {
            $this->warning('Catatan penolakan tidak boleh kosong.');
            return;
        }

        $this->prosesTolak(trim($this->catatanTolak));

        // Bersihkan state modal setelah ditolak
        $this->modalTolak = false;
        $this->catatanTolak = '';
    }

    // Dipisahkan agar tidak duplikasi penulisan
    private function prosesTolak(string $catatanText): void
    {
        if (!$this->pegawai || empty($this->availableFiles)) return;
        ...
        // 2. Tampilkan Notifikasi Toast ke Verifikator
        $this->warning("{$config['label']} ditolak: $catatanText");

        // 3. Lanjut ke dokumen atau pegawai selanjutnya
        $this->nextFileOrPegawai();
    }
...
<div class="flex flex-col h-[calc(100vh-40px)] -mx-4 -mt-4 overflow-hidden bg-base-200">
    ...
    <div class="flex-1 flex overflow-hidden">
        ...
        @if($pegawai && !empty($availableFiles))
            ...
            >
                ...
                <div
                        wire:loading.flex
                        wire:target="nextPegawai,prevPegawai,loadPendingNips,cariNip,approve,tolakDenganCatatan,simpanPenolakan"
            ...
            <div class="w-88 lg:w-96 border-l border-base-300 flex flex-col shrink-0 h-full shadow-2xl z-10">
                ...
                <div class="flex-1 overflow-y-auto p-5 flex flex-col gap-5">
                    ...
                    <div class="flex flex-col gap-2.5">
                        {{-- Logika Penampilan Tombol Berdasarkan Tipe Berkas --}}
                        @if(isset($config['is_custom_catatan']) && $config['is_custom_catatan'])
                            <button wire:click="$set('modalTolak', true)" wire:loading.attr="disabled" class="btn btn-outline btn-error h-auto min-h-12 py-2 px-4 justify-center text-center text-sm hover:scale-[1.01] transition-transform border-base-300 hover:border-error bg-base-100 shadow-sm font-bold">
                                <x-icon name="o-pencil-square" class="w-5 h-5 mr-2 shrink-0" />
                                <span class="leading-snug">Ketik Catatan Penolakan</span>
                            </button>
                        @else
    ...
    {{-- Modal Mary UI untuk Catatan Penolakan Custom (SKP) --}}
    <x-modal wire:model="modalTolak" title="Tolak Dokumen" subtitle="Berikan alasan spesifik mengapa SKP ditolak" separator>
        <div class="py-2">
            <x-textarea
                    label="Catatan Kesalahan / Revisi"
                    wire:model="catatanTolak"
                    placeholder="Ketik detail kesalahan pada SKP atau apa yang harus diperbaiki oleh pegawai..."
                    rows="4"
                    class="w-full"
            />
        </div>
        <x-slot:actions>
            <x-button label="Batal" wire:click="$set('modalTolak', false)" class="btn-ghost" />
            <x-button label="Tolak & Simpan Catatan" wire:click="simpanPenolakan" class="btn-error text-white" icon="o-paper-airplane" spinner="simpanPenolakan" />
        </x-slot:actions>
    </x-modal>
